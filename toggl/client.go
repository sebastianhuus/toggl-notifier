...
// ElapsedSeconds returns the entry's elapsed time. For running entries
// (duration < 0), it computes now - start instead of trusting the raw value.
...
// EntriesForDay returns entries that fall within the local-time day, filtered
// to the configured workspace and project.
func (c *Client) EntriesForDay(ctx context.Context, day time.Time) ([]TimeEntry, error) {
	...
	reqURL := fmt.Sprintf("%s/me/time_entries?%s", baseURL, params.Encode())
...
// TotalSeconds sums elapsed seconds across the given entries.
